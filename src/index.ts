...
async function  teste(){
  const createdLoad = await orchBase.getLoads(null, {
    id_load: 5,
  })

  console.log(createdLoad);

  const createdQueue = await orchBase.getQueues(null, {
    id_load: 5,
  })
  ...
  const createdContact = await orchBase.getContacts(null, {
    id_contact: 5,
  })
  ...
  const createdContactPhone = await orchBase.getContactsPhone( null, {
    id_contact: 5,
  })

  console.log(createdContactPhone);

  const createdContactEmail = await orchBase.getContactsEmail( null, {
    id_contact: 5,
  })

  console.log(createdContactEmail);

  const createdContactComplement = await orchBase.getContactsComplement( null, {
    id_contact: 5,
  })

  console.log(createdContactComplement);
